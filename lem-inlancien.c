...
/*   Updated: 2018/05/10 16:55:57 by mpascaud         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int		ft_check_prev_place(t_roomlist *roomlist, int i, t_roomlist *roomlistart)
{
	int		empty;
...
void	ft_place(t_filist *filist, t_roomlist* roomlist)
{
	...
	while (roomlist != NULL)
	{
		while ((roomlist->beftunnels)[i] != NULL)
		{
			if (ft_check_prev_place(roomlist, i, roomlistart) == 1)
			{
				roomlist = roomlistart;
				//roomlist = roomlist->next;
				break ;
...
//pour prev : que la place du prev = place du suivant - 1;
...
int		ft_search_prev(t_roomlist *roomlist, t_roomlist *roomlistart, int way)
{
	t_roomlist	*tmp;
...
int		ft_way(t_filist *filist, t_roomlist *roomlist, t_roomlist *roomlistart, int way)
{
	...
	while (/*ft_no_zero(roomlistart) == 0*/1/*roomlist != NULL*/)//penser au -2(et au 0)
	{
		while (roomlist != NULL)
		{
			/*if (roomlist->place == -2 && ft_search_prev(roomlist, roomlistart, way) == 1000)
			{
			}*/
			if (roomlist->way == 0 && roomlist->place != 0 && roomlist->taken == 0 && ft_search_prev(roomlist, roomlistart, way) == 1)
			{
			...
			if (/*!!!!!!!*/(roomlist->way == way || roomlist->way == 0) && roomlist->place != 0 && roomlist->place != -2 && (ft_search_next(roomlist, roomlistart, way) == 0 || ft_search_prev(roomlist, roomlistart, way) == -1))//si pas de connexion avec un +1, ou qu'avec des -1
			{
